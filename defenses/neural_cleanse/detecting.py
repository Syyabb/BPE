...
class RegressionModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = (1 - mask) * x + mask * (0.9 * x + 0.1 * pattern)
    ...
    def _get_classifier(self, opt):
        ...
        classifier.load_state_dict(torch.load('/data1/bingxu/pycharm_ssh/backdoor_sota/warp_and_blend_uap/blend.pt'))
        # classifier.load_state_dict(torch.load('/data1/bingxu/pycharm_ssh/backdoor_sota/warp_and_blend_uap/patch.pt'))
        # classifier.load_state_dict(torch.load('/data1/bingxu/pycharm_ssh/backdoor_sota/warp_and_blend_uap/sig.pt'))

        # from classifier_models.resnet_cifar import resnet18
        # classifier = resnet18(10).to(opt.device)
        # classifier.load_state_dict(torch.load('/data1/bingxu/pycharm_ssh/backdoor_sota/ANP/resnet18/model_last.th'))
...
class Recorder:
    ...
    def save_result_to_dir(self, opt):
        ...
        path_pattern = os.path.join(result_dir, "blend_img.png")
        ...
        path_mask = os.path.join(result_dir, "mask.pt")
        path_pattern = os.path.join(result_dir, "blend_img.pt")
        path_trigger = os.path.join(result_dir, "trigger.pt")

        torch.save(mask_best, path_mask)
        torch.save(pattern_best, path_pattern)
        torch.save(trigger, path_trigger)
